...
I find it's helpful to start off with just giving you a little bit of my background. I have graduate degrees in Statistics, but my undergrad was in math and psychology, and my dissertation could have probably been in psychology if my advisor had been a psychologist instead of a statistician. The coursework might have been more enjoyable, at the very least... :)
...
Before we start in on the real meat of the discussion, I'm going to run through a really quick primer on forensics and forensic evidence. How many of you are familiar with the 2010 NAS Report? What about the 2016 PCAST report? These reports are a critical piece of the discussion about forensics and error-rate testing. 

At any point, if you all know all of this, just interrupt me. I wasn't sure what the background of this group was going to be, so I just included most of the stuff, and we can pick and choose from that what we want to actually talk about. There's definitely plenty of discussion potential around the issues I'm going to bring up here. 
...
Today, I thought I'd start with a study that grew out of a friend being called to testify as a judge's expert in a NY State court case involving firearms evidence. To prepare to testify, she looked at a bunch of studies and was struck by both the poor quality of the studies and the decisions made by the examiners. 
...
I started all of this without knowing a darn thing about guns, so I'll briefly talk about how this all works before discussing the basic principles of firearms examination
...
```{r, echo = F, message = F, warning = F, fig.cap = "The distribution of inconclusives should be equivalent to the ratio of same-source and different-source comparisons", out.width = "80%", fig.width = 6, fig.height = 4}
source("make_inconcl_plot.R")
target_plot(200, 50, 10, 10, 50, 200)
```
...
```{r, echo = F, message = F, warning = F, fig.cap = "The distribution of inconclusives should be equivalent to the ratio of same-source and different-source comparisons", out.width = "60%", out.extra="style='position: absolute;left:10px;'", fig.width = 6, fig.height = 4}
# source("make_inconcl_plot.R")
div <- 4
target_plot(round(1075/div), round(11/div), round(4/div), round(22/div), round(737/div), round(1421/div))
```
.right-margin[
